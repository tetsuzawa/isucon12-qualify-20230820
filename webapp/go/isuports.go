...
type CompetitionCompetitionReportRow struct {
	TenantID   int64         `db:"tenant_id"`
	ID         string        `db:"id"`
	Title      string        `db:"title"`
	FinishedAt sql.NullInt64 `db:"finished_at"`
	...
	CompetitionReportID string `json:"competition_report_id" db:"competition_report_id"`
...
func billingReportByCompetition(ctx context.Context, tenantDB *sqlx.DB, tenantID int64, competitonID string) (*BillingReport, error) {
	...
	br := &BillingReport{}
	err = tenantDB.GetContext(ctx, br, "SELECT * FROM competition_report WHERE competition_id = ?", competitonID)
	if errors.Is(err, sql.ErrNoRows) {
...
func billingReportsByCompetitionIDs(ctx context.Context, tenantDB *sqlx.DB, tenantID int64) (int64, error) {
	tx, err := tenantDB.Beginx()
	if err != nil {
		return 0, fmt.Errorf("error bigin: %w", err)
	}
	defer tx.Commit()

	var cs []CompetitionCompetitionReportRow
	cids := make([]string, 0, len(cs))
	err = tx.SelectContext(ctx, &cs, "SELECT competition.* FROM competition LEFT JOIN competition_report ON competition.tenant_id = ? AND competition.id = competition_report.competition_id WHERE competition.tenant_id = ? AND competition.finished_at IS NOT NULL AND competition_report.id IS NULL", tenantID, tenantID)
	if err != nil {
		return 0, fmt.Errorf("error Select competition: tenantID=%d, %w", tenantID, err)
	}
	for _, c := range cs {
		cids = append(cids, c.ID)
		_, err = generateBillingReportWithCompetition(ctx, tenantDB, tenantID, CompetitionRow{
			ID:         c.ID,
			Title:      c.Title,
			TenantID:   tenantID,
		...
		if err != nil {
			return 0, fmt.Errorf("error generateBillingReport: %w", err)
		}
	}

	var billingYen int64
	err = tx.GetContext(ctx, &billingYen, "SELECT CASE WHEN SUM(billing_yen) IS NOT NULL THEN SUM(billing_yen) ELSE 0 END FROM competition_report INNER JOIN competition ON competition.tenant_id = ? AND competition_report.competition_id = competition.id WHERE competition.tenant_id = ?", tenantID, tenantID)
	if err != nil {
...
func generateBillingReportWithCompetition(ctx context.Context, tenantDB *sqlx.DB, tenantID int64, comp CompetitionRow) (*BillingReport, error) {
	// ランキングにアクセスした参加者のIDを取得する
	vhs := []VisitHistorySummaryRow{}
	if err := adminDB.SelectContext(
		ctx,
...
func tenantsBillingHandler(c echo.Context) error {
	...
	for _, t := range ts {
		...
		err := func(t TenantRow) error {
			...
			//}
			billingYen, err := billingReportsByCompetitionIDs(ctx, tenantDB, t.ID)
			if err != nil {
				return fmt.Errorf("failed to billingReportsByCompetitionIDs: %w", err)
			}
