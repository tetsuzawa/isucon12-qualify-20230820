...
func billingReportsByCompetitionIDs(ctx context.Context, tenantDB *sqlx.DB, tenantID int64) (int64, error) {
	...
	err = tx.SelectContext(ctx, &cs, "SELECT competition.id, competition.finished_at FROM competition LEFT JOIN competition_report ON competition.tenant_id = ? AND  competition.finished_at IS NOT NULL AND competition.is = competition_report.competition_id WHERE competition_report.id IS NOT NULL", tenantID)
	if err != nil {
