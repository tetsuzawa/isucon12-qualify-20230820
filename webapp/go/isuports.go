...
func billingReportsByCompetitionIDs(ctx context.Context, tenantDB *sqlx.DB, tenantID int64) (int64, error) {
	...
	err = tx.SelectContext(ctx, &cs, "SELECT competitoin.* FROM competition LEFT JOIN competition_report ON competition.tenant_id = ? AND competition.id = competition_report.competition_id WHERE competition.tenant_id = ? AND competition.finished_at IS NOT NULL AND competition_report.id IS NULL", tenantID, tenantID)
	if err != nil {
